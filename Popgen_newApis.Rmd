...
beePi <- read.csv("results/apis_reordered.gene.pi.csv")
beeSub <- read.csv("results/substitutions_reordered.csv")

antEvol <- read.csv("data/MpharAnn.csv")
antConstraint <- read.table("results/MKtest_constraint_ant.csv")
beeSnipre <- read.csv("results/snipre_apis_reordered.csv")
...
map = read.table("results/apis_map_reordered_edit.txt",head=F)
colnames(map) = c("Gene","old_gene")
beeSub = merge(beeSub,map,by="Gene")
beePi = merge(beePi,map,by="Gene")
beePi$Gene = beePi$old_gene
beePi = beePi[,-c(3)]

beeConstraint = read.table("results/MKtest_globalAlpha_locusF_bee_reordered")
...
beeAnn = read.csv("results/annotation_reordered.csv",header=F) 
...
beeSub$Gene = beeSub$old_gene
beeSub = beeSub[,colnames(beeSub) != "old_gene"]
...
```{r alpha}
# alpha <- read.csv("results/collectedAlpha.csv")
# colnames(alpha)[1:3] = c("nonDE","queen","worker")
# alphaM = melt(alpha,id.vars =c("stage","species"))
# colnames(alphaM)[3:4] = c("value","alpha")
# aD = calcMean(alphaM,"alpha")
# 
# ggplot(aD,aes(x = stage,fill=value,y=mean))+
#   geom_bar(stat="identity",position=position_dodge())+
#   geom_errorbar(aes(ymin=c1,ymax=c2),position=position_dodge(width=0.9),width=0.3)+  
#   facet_wrap(. ~ species)+
#   ylab("alpha")+
